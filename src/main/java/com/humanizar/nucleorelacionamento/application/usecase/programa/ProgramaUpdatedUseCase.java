...
@Component
public class ProgramaUpdatedUseCase {

    private static final Logger log = LoggerFactory.getLogger(ProgramaUpdatedUseCase.class);

    private final AbordagemPatientService abordagemPatientService;
    private final ProcessedEventPort processedEventPort;

    public ProgramaUpdatedUseCase(
    ...
    public EventOutcome execute(
        ...
        try {
            abordagemPatientService.reconcileAbordagens(payload, envelope.correlationId());
    ...
    private EventOutcome handleDomainException(
        ...
        ReasonCode reasonCodeCreated = ex.getReasonCode() != null
                ? ex.getReasonCode()
                : ReasonCode.VALIDATION_ERROR;

        if (reasonCodeCreated == ReasonCode.DUPLICATE_EVENT) {
            ...
            saveProcessedEvent(buildProcessedEvent(
                    ...
                    reasonCodeCreated,
                    reasonCodeCreated.name().toLowerCase()));
            return EventOutcome.ignored(reasonCodeCreated);
        }

        if (reasonCodeCreated.isRetryable()) {
            log.error("Falha retentavel. reasonCode={}, eventId={}, correlationId={}, consumer={}",
                    reasonCodeCreated, envelope.eventId(), envelope.correlationId(), consumerName, ex);
            return EventOutcome.failed(reasonCodeCreated);
        }

        log.warn("Falha funcional nao retentavel. reasonCode={}, eventId={}, correlationId={}, consumer={}",
                reasonCodeCreated, envelope.eventId(), envelope.correlationId(), consumerName);
        saveProcessedEvent(buildProcessedEvent(
                consumerName,
                envelope,
                routingKey,
