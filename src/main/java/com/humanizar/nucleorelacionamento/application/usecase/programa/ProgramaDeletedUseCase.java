...
public class ProgramaDeletedUseCase {
    ...
    private static final Logger log = LoggerFactory.getLogger(ProgramaDeletedUseCase.class);
    ...
    public EventOutcome execute(
        ...
        try {
            abordagemPatientService.deleteAllAbordagensByPatientId(command.patientId(), envelope.correlationId());
    ...
    private EventOutcome handleDomainException(
        ...
        ReasonCode reasonCodeDeleted = ex.getReasonCode() != null
                ? ex.getReasonCode()
                : ReasonCode.VALIDATION_ERROR;

        if (reasonCodeDeleted == ReasonCode.DUPLICATE_EVENT) {
            ...
            saveProcessedEvent(buildProcessedEvent(
                    ...
                    reasonCodeDeleted,
                    reasonCodeDeleted.name().toLowerCase()));
            return EventOutcome.ignored(reasonCodeDeleted);
        }

        if (reasonCodeDeleted.isRetryable()) {
            log.error("Falha retentavel. reasonCode={}, eventId={}, correlationId={}, consumer={}",
                    reasonCodeDeleted, envelope.eventId(), envelope.correlationId(), consumerName, ex);
            return EventOutcome.failed(reasonCodeDeleted);
        }

        log.warn("Falha funcional nao retentavel. reasonCode={}, eventId={}, correlationId={}, consumer={}",
                reasonCodeDeleted, envelope.eventId(), envelope.correlationId(), consumerName);
        saveProcessedEvent(buildProcessedEvent(
                consumerName,
                envelope,
                routingKey,
    ...
    private EventOutcome handleUnexpectedException(
        ...
        log.error(
                "Excecao inesperada. eventId={}, correlationId={}, consumer={}, eventType={}, aggregateType={}, aggregateId={}",
                envelope.eventId(), envelope.correlationId(), consumerName,
