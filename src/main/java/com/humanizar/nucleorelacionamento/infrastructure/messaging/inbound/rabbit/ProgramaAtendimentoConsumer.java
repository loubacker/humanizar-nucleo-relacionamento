...
public class ProgramaAtendimentoConsumer {
    ...
    private final Map<String, ProgramaRoutingHandler> routingHandlers;
    private final RabbitAcknowledgementConfig rabbitAcknowledgementConfig;

    public ProgramaAtendimentoConsumer(
            ...
            List<ProgramaRoutingHandler> routingHandlers,
        ...
        this.routingHandlers = routingHandlers.stream()
                .collect(Collectors.toUnmodifiableMap(ProgramaRoutingHandler::routingKey, Function.identity()));
    ...
    public void onMessage(Message message, Channel channel) throws IOException {
        ...
        try {
            ...
            if (ex.getReasonCode() == ReasonCode.INBOUND_PARSE_ERROR) {
                log.error("Falha de parse no consumer programa. {}", context, ex);
            ...
            if (ex.isRetryable()) {
                ...
                log.warn("Erro nao retentavel no consumer programa. reason={}, {}", ex.getReasonCode(), context);
                tryPublishEarlyRejection(envelope, routingKey, ex);
                rabbitAcknowledgementConfig.ack(channel, deliveryTag, context);
            }
        } catch (RuntimeException ex) {
            log.error("Erro inesperado no consumer programa. {}", context, ex);
            rabbitAcknowledgementConfig.nackRetry(channel, deliveryTag, context);
        }
    }
    ...
    private EventOutcome dispatchByRoutingKey(
            String routingKey,
            InboundEnvelopeDTO<Object> envelope) {
        ProgramaRoutingHandler handler = routingHandlers.get(routingKey);
        if (handler == null) {
            String correlationId = envelope.correlationId() != null
                    ? envelope.correlationId().toString()
                    : null;
